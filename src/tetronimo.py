...
class Tetronimo:
    ...
    def try_rotate(self, board, rotations):
        ...
        next_rotation = tetronimoData.Rotation(
            (self.current_rotation.value + rotations)
            % tetronimoData.possible_rotation_count
        )
        next_position = self.position

        if self.check_collision(board, next_position, next_piece_data):
            kick_table = (
                tetronimoData.kick_table_I
                if self.piece == tetronimoData.Tetronimoes.I
                else tetronimoData.kick_table
            )
            for kick_data in kick_table[
                (self.current_rotation.value, next_rotation.value)
            ]:
                new_position_to_check = (
                    next_position[0] - kick_data[1],
                    next_position[1] + kick_data[0],
                )
                if not self.check_collision(
                    board, new_position_to_check, next_piece_data
                ):
                    self.do_rotation(
                        next_piece_data, next_rotation, new_position_to_check
                    )
                    return True
            return False
    ...
    def ready_to_lock(self, lock_ticks, lock_max_rotations):
        if (
            self.lock_tick_counter > lock_ticks
            or self.rotations_since_failed_drop > lock_max_rotations
        ):
            return True
        return False
    ...
    def draw_piece(self, surface, board_pos, piece_data, position, alpha=255):
        for row in range(piece_data.shape[0]):
            for col in range(piece_data.shape[1]):
                ...
                colour = tetronimoData.tetronimo_colours[
                    tetronimoData.Tetronimoes(cell_state)
                ]
        ...
        if self.settings["debug"]:
            ...
            pg.draw.rect(
                surface,
                (0, 255, 0),
                (start_pos, (bounding_box_size, bounding_box_size)),
                2,
            )
